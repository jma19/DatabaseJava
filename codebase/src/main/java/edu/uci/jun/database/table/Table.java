...
public class Table implements Iterable<Record>, Closeable {
    ...
    private void readHeaderPage() {
        ...
        List<DataFiled> fieldTypes = new ArrayList<DataFiled>();
        for (int i = 0; i < numFields; i++) {
            int ordinal = ByteBuffer.wrap(headerPage.readBytes(numBytesRead, 4)).getInt();
            DataFiled.Types type = DataFiled.Types.values()[ordinal];
            numBytesRead += 4;

            switch (type) {
                case INT:
                    fieldTypes.add(new IntDataField());
                    break;
                case STRING:
                    int len = ByteBuffer.wrap(headerPage.readBytes(numBytesRead, 4)).getInt();
                    numBytesRead += 4;

                    fieldTypes.add(new StringDataField(len));
                    break;
                case BOOL:
                    fieldTypes.add(new BoolDataFiled());
                    break;
                case FLOAT:
                    fieldTypes.add(new FloatDataField());
                    break;
            }
        }
